...
def main():
    ...
    logger.info(f"Загружено {len(search_words_set)} слов из словаря: {', '.join(search_words_set)}")
    ...
    for i, transcription in enumerate(transcriptions):
        ...
        word_duration = segment_duration / len(words) if words else 0  # Длительность одного слова

        for word_index, word in enumerate(words):
            clean_word = word.strip(string.punctuation).lower()

            if clean_word in search_words_set:
                word_start_time = segment_start + word_index * word_duration
                word_end_time = word_start_time + word_duration

                logger.info(f"Найдено слово '{clean_word}' в сегменте {i} "
                            f"(начало: {word_start_time:.2f}, конец: {word_end_time:.2f})")
    ...
    with open(csv_file_path, mode="w", newline="", encoding="utf-8") as file:
        ...
        for word, start_time, end_time in search_results:
            duration = "00;00;00;00"  # Для слов продолжительность можно оставить 0
            formatted_start = format_time(start_time)
            formatted_end = format_time(end_time)
            writer.writerow(
                [word, "", formatted_start, formatted_end, duration, "Comment"]
            )
